...
from src.whatsapp.handlers.update_handler import (
    ...
    show_master_data as _show_master_data,
...
async def handle_owner(
    ...
    handlers = {
        ...
        "SHOW_MASTER_DATA":   _show_master_data,
...
async def _build_checkout_summary(action_data: dict, phone: str, session: "AsyncSession") -> str:
    ...
    if nd is None:
        ...
        notice_line = (
            f"\nNotice: {nd.strftime('%d %b %Y')} "
            f"(on time, on/before {_NOTICE_BY_DAY}th) — deposit refundable"
        )
    else:
        notice_line = (
            f"\nNotice: {nd.strftime('%d %b %Y')} "
            f"(after {_NOTICE_BY_DAY}th — next month's cycle, full month rent required) "
            f"— *deposit forfeited*"
        )
...
async def resolve_pending_action(
    ...
    # Dispatch table — resolvers migrated out of this function in Phase 2.
    # Any intent registered in PENDING_RESOLVERS returns here. Unregistered
    # intents fall through to the legacy if/elif chain below. See plan:
    # docs/superpowers/plans/2026-04-23-phase2-handler-refactor.md
    from src.whatsapp.handlers.resolvers import PENDING_RESOLVERS
    if resolver := PENDING_RESOLVERS.get(pending.intent):
        return await resolver(
            pending, reply_text, session, action_data, choices,
            media_id=media_id, media_type=media_type, media_mime=media_mime,
        )
    ...
    # APPROVE_ONBOARDING migrated to resolvers/onboarding.py (Phase 2A,
    # commit {see git log}). Dispatch at top of this function routes it.
    ...
    if chosen is not None and pending.intent in ("CHECKOUT", "SCHEDULE_CHECKOUT"):
        ...
        if nd is None:
            ...
            notice_line = (
                f"Notice: {nd.strftime('%d %b %Y')} (on time ✓) — deposit refundable"
            )
        else:
            notice_line = (
                f"Notice: {nd.strftime('%d %b %Y')} "
                f"(after {_NOTICE_BY_DAY}th — must stay till end of next month) — *deposit forfeited*"
            )
...
async def _do_checkout(
    ...
    if eligible:
        ...
        # Deposit forfeited — no notice OR late notice (after cutoff).
        # Extra-month penalty applies only to walk-outs with zero notice.
        extra_month = (tenancy.agreed_rent or Decimal("0")) if no_notice else Decimal("0")
        net = Decimal("0") - o_rent - o_maintenance - extra_month

    settlement_lines = ["\n*Settlement Summary*"]
    settlement_lines.append(f"Security deposit held : Rs.{int(deposit):,}")
    if not eligible:
        forfeit_reason = "no notice given" if no_notice else f"late notice — after {_NOTICE_BY_DAY}th"
        settlement_lines.append(f"Deposit forfeited     : -Rs.{int(deposit):,} ({forfeit_reason})")
    if extra_month > 0:
...
async def _notice_given(entities: dict, ctx: CallerContext, session: AsyncSession) -> str:
    ...
    if notice_date_val.day <= _NOTICE_BY_DAY:
        ...
        deposit_note = (
            f"After {_NOTICE_BY_DAY}th — must stay till end of next month, full month's rent required.\n"
            f"Vacate date: *{last_day.strftime('%d %b %Y')}*. *Deposit forfeited.*"
        )
