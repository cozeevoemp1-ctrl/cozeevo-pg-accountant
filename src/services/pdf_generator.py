...
HOUSE_RULES = [
    "Vacating notice must be given on or before the 5th of the month. Notice given on/before the 5th: tenant may vacate by month end. Notice given after the 5th: applies to the next month's cycle, full month's rent required. The security deposit is refundable as long as vacating notice was given — it is forfeited only if no notice is given at all. (Note: should vacate only on 30th or 31st of any month)",
