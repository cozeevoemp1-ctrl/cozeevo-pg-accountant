...
import { fmtDate, monthLabel, periodMonth } from "@/lib/date";
  ...
  return (
    <main className="min-h-screen bg-bg pb-32">
      ...
      <div className="px-4 pt-4 flex flex-col gap-3 max-w-lg mx-auto">
        {/* Month picker */}
        <div className="bg-surface rounded-card border border-border px-3 py-2.5">
        ...
        {!loading && filtered.length > 0 && (
          <div className="flex gap-2">
            <div className="flex-1 bg-surface rounded-card border border-border px-3 py-2 text-center">
            ...
            <div className="flex-1 bg-surface rounded-card border border-border px-3 py-2 text-center">
              ...
              <p className="text-lg font-extrabold text-status-paid">
                {rupee(filtered.reduce((s, i) => s + i.refund_amount, 0))}
              </p>
            </div>
          </div>
        )}
        ...
        {loading && (
          <div className="flex flex-col gap-3">
            {[1, 2, 3].map(i => (
              <div key={i} className="bg-surface rounded-card border border-border h-20 animate-pulse" />
            ))}
          </div>
        )}
        ...
        {!loading && filtered.map(item => (
          <div key={item.tenancy_id} className="bg-surface rounded-card border border-border px-4 py-3 flex flex-col gap-2">
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
              ...
              <div>
                ...
                <p className="font-semibold text-ink">{rupee(item.security_deposit)}</p>
              </div>
              <div>
                <p className="text-ink-muted">{item.stay_type === "daily" ? "Daily rate" : "Agreed rent"}</p>
                <p className="font-semibold text-ink">{rupee(item.agreed_rent)}{item.stay_type === "daily" ? "/day" : "/mo"}</p>
              </div>
              <div>
                <p className="text-ink-muted">Refund</p>
                <p className={`font-semibold ${item.refund_amount > 0 ? "text-status-paid" : "text-ink-muted"}`}>
                  {item.refund_amount > 0 ? rupee(item.refund_amount) : "—"}
                </p>
              </div>
            </div>
          </div>
