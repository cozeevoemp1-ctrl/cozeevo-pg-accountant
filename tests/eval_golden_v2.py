"""
tests/eval_golden_v2.py
========================
Golden Dataset Evaluator for the v2 Supervisor Agent pipeline.

Same 100 golden cases as eval_golden.py, but posts to:
    POST /api/v2/whatsapp/process

Target: >= 93% accuracy (same baseline as v1), ideally higher due to
conversation memory context.

Usage:
    python tests/eval_golden_v2.py                          # all 100 cases
    python tests/eval_golden_v2.py --category correction_mid_flow
    python tests/eval_golden_v2.py --id G001
    python tests/eval_golden_v2.py --check-leaks            # only check reply_must_not_contain
    python tests/eval_golden_v2.py --verbose                # show all turn details
...
API_URL     = f"http://localhost:{API_PORT}/api/v2/whatsapp/process"   # v2 endpoint
...
REQUEST_TIMEOUT = 20   # v2 has LLM call — allow extra time
INTER_TURN_DELAY_MS = 300   # slightly higher than v1 for LLM latency
...
def _check_turn(turn: dict, response: dict) -> tuple[str, list[str]]:
    """Evaluate a single turn response. Returns (outcome, list_of_failure_reasons)."""
    failures: list[str] = []
    reply = response.get("reply", "").lower()
    got_intent = response.get("intent", "")

    # 1. Technical leak check
    leaks_found = []
    ...
    for i, turn in enumerate(turns):
        ...
        try:
            resp = await client.post(
                API_URL,
                json={"phone": phone, "message": msg, "message_id": f"goldenV2-{case_id}-t{i}"},
        ...
        if verbose:
            icon = "v" if outcome == PASS else ("!" if outcome == LEAK else "x")
            print(f"    [{icon}] T{i+1} ({role}): {msg!r}")
            print(f"          intent={data.get('intent')} conf={confidence:.2f}")
            if outcome != PASS:
                print(f"          FAIL: {reason}")
...
async def run_all(
    ...
    async with httpx.AsyncClient() as client:
        for i, case in enumerate(cases):
            print(f"  [{i+1:3d}/{len(cases)}] {case['id']} -- {case['name'][:55]}", end="", flush=True)
            result = await run_case(case, client, verbose=verbose, check_leaks_only=check_leaks_only)
            icon = "v" if result["outcome"] == PASS else ("!" if result["outcome"] == LEAK else "x")
            print(f"  {icon}")
...
def print_report(results: list[dict], elapsed: float) -> bool:
    ...
    print("\n" + "=" * 65)
    print("  GOLDEN DATASET RESULTS — v2 Supervisor Agent")
    print("=" * 65)
    ...
    print(f"  Time        : {elapsed:.1f}s  (avg {elapsed/total:.1f}s/case)")
    print()
    ...
    print()
    if passed == total:
        print("  [PASS] GOLDEN SUITE PASSED -- v2 safe to deploy to VPS")
    elif leaked > 0:
        print("  [BLOCK] TECHNICAL LEAKS DETECTED -- BLOCK DEPLOY immediately")
    else:
        pct = passed / total * 100
        v1_baseline = 93
        vs = "above" if pct >= v1_baseline else "below"
        print(f"  [FAIL] {failed + leaked} cases failing ({pct:.1f}% -- {vs} v1 {v1_baseline}% baseline)")
    print("=" * 65)
...
def main() -> None:
    ...
    out_path = args.out or str(RESULTS_DIR / f"{ts}_golden_v2_results.json")
