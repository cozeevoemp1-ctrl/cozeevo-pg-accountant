...
| `scripts/_add_settlement_payments_sept.py` | One-off: appends settlement rent receipts to **exited** tenancies via `log_payment()`. Reference recipe for the guard that deletes the phantom `RentSchedule` row `log_payment()` auto-creates when the period has none (it seeds `rent_due` = full agreed rent, inventing a due for someone who already left). |
